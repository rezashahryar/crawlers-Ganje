...
def normalize_item(item, ad_type):
    for k, v in item.items():
        ...
        normalize_trovit_item(item)

def normalize_trovit_item(item):
    item['price'] = normalize_numerical_field(item['price'])
    item['rent'] = normalize_numerical_field(item['rent'])
    item['room'] = normalize_numerical_field(item['room'])
    item['area'] = normalize_numerical_field(item['area'])
    item['bathroom'] = normalize_numerical_field(item['bathroom'])

def normalize_numerical_field(field):
    changed_filed = str(field).replace('m²', '')
    changed_filed = str(changed_filed).replace(' ', '')
    return clean_number(changed_filed)
