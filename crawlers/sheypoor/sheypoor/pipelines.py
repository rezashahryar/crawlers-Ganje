...
from sheypoor.utilities.Normalize import normalize_item
from sheypoor.utilities.configs import server_db, local_db


class SheypoorPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        # if isinstance(item, SheypoorItemVehicle):
        ...
        try:
            if self.insertThis(item["title"]):
                ...
                (("token," if self.insertThis(item["token"]) else "") +
                 ...
                 ("city_slug," if self.insertThis(item["city_slug"]) else "") +
                ...
                ((f"{item['token']}," if self.insertThis(item["token"]) else "") +
                 ...
                 (f"'{item['city_slug']}'," if self.insertThis(item["city_slug"]) else "") +
    ...
    def save_car_data(self, item):
        try:
            ...
            self.conn.rollback()
            logger.critical(f"storing car failed: {e} + {item['url']}")
        return item
