...
from sheypoor.utilities.Normalize import remove_extra_character_and_normalize
from sheypoor.utilities.Normalize import normalize_text
...
def get_province(city: str):
    ...
    unusual = {
        ...
        'قاسم‌آباد (خواف)': {"p": "مشهد", "c": "قاسم‌آباد (خواف)"},
...
def _get_city_slug(province: str, city: str):
    try:
        cursor.execute(
            f"""select api_city.slug 
                from api_city inner join api_province on api_city.province_id=api_province.id  
                where api_province.name='{province}' and api_city.name='{city}'"""
        )
        data = cursor.fetchall()
        return data[0][0]
    except Exception as e:
        conn.rollback()
        logger.critical(f"get_city_slug for {province} {city}: {e}")
        return None


def get_city_slug(province: str, city: str):
    slug = _get_city_slug(province, city)
    return province, city, slug
