...
def normalize_item(item, ad_type):
    ...
    elif 'recruiment' == ad_type:
        normalize_recruitment(item)

def normalize_recruitment(item):
    item['cooperation'] = normalize_cooperation(item['cooperation'])
    item['education'] = normalize_eduacation(item['education'])

def normalize_cooperation(cooperation: str):
    if 'تمام' in cooperation or 'امریه' in cooperation or 'بازنشسته' in cooperation:
        return 'تمام وقت'
    elif 'نیمه' in cooperation or 'پاره' in cooperation or 'دور' in cooperation:
        return 'پاره وقت'
    elif 'آموز' in cooperation:
        return 'کارآموز'
    elif 'پروژه' in cooperation or 'پیمان' in cooperation:
        return 'پروژه ای'
    return None

def normalize_eduacation(education: str):
    if 'ارشد' in education or ('فوق' and 'لیسانس' in education):
        return 'فوق لیسانس'
    elif 'کاردان' in education or ('فوق' and 'دیپلم' in education):
        return 'فوق دیپلم'
    elif 'کارشناس' in education or 'لیسانس' in education:
        return 'لیسانس'
    elif 'دیپلم' in education:
        return 'دیپلم'
    elif 'دکتر' in education or 'پی اچ دی' in education:
        return 'دکتری'
    elif 'دانشجو' in education:
        return 'دانشجو'
    return None
...
def normalize_car_item(item):
    ...
    item['price'] = clean_number(item['price'], int_type=False)
...
def normalize_room(number: str):
    data = clean_number(number, False)
    if data != "":
       return data
    elif data == "":
