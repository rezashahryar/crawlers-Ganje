...
from divar.utilities.Normalize import clean_number, remove_extra_character_and_normalize

from divar.utilities.uses import get_production, get_time_stamp, hash_token, get_persian_year, extract_model_brand
...
class HamrahmechanicCarItem(CarBaseItem, BaseItem):
    def extract(self, response, car_dict):
        self['token'] = hash_token(response.request.url.split('/')[-2], 9)
        self['source_id'] = 9
        self['time'] = get_time_stamp()
        self['thumbnail'] = car_dict['imageUrl']
        self['title'] =response.css('[class^="CarInformation_car-information__brand-name"]::text').extract_first()
        self['brand'] = response.css('[class^="CarInformation_car-information__brand-name"]::text').extract_first()
        self['price'] = car_dict['price'] or "not_defined"
        self['consumption'] = str(car_dict['km']) or "not_defined"
        self['gear_box'] = car_dict['gearBoxPersian'] or "not_defined"
        self['color'] = response.css('[class^="CarTechnicalDetails_car-details__item-value"]::text').extract()[4]
        self['category'] = 'خودرو'
        self['sub_category'] = 'سواری'
        self['description'] = response.css('[class^="expandable-text_excerpt"]::text').extract()[1] or "not_defined"
